import{d as e,n as t,t as n}from"./fs-access-Cjcg0_Me-BYL2BwWI.js";import{B as r,D as i,H as a,U as o,ft as s,i as c}from"./dist-DPdFd7_Q.js";import"./src-Bli3e1fu.js";import{t as l}from"./contributions-3pl8RjZl.js";import"./lit-DAQWYrzS.js";import{n as u}from"./api-BHnM_fFQ.js";import{t as d}from"./pyservice-0zxWD96J-CmpcitIN.js";import"./api-DW0JU5Kh.js";var f=`import os
...
`,m=`"""Workspace VFS bridge for ancpBIDS in Pyodide (uses Eclipse Docks \`\`docks\`\` module)."""
import posixpath
...
`,g=`def _normalize_options(options):
    if not options:
        return {}
...
`,_=`def _normalize_options(options):
    if not options:
        return {}
    if hasattr(options, "to_py"):
        return dict(options.to_py())
...
`,v=f,y=p,b=m,x=h,S=g,C=_,w=`[ancpBIDS]`;function T(e,t,n){let r=n?` (${n})`:``;console.log(`${w} ${e}: ${t}ms${r}`)}async function E(e,t,n){let r=performance.now(),i=await t();return T(e,Math.round(performance.now()-r),n?.(i)),i}async function D(e){let r=performance.now(),i=[],a=0,o=0,s=0,c=async(e,r)=>{s+=1;let l=performance.now(),u=await e.listChildren(!1),d=Math.round(performance.now()-l);if(d>=250){let t=r||e.getName()||`/`;console.log(`[ancpBIDS] slow listChildren: ${d}ms at ${t} (${u.length} entries)`)}for(let e of u){if(e instanceof n){a+=1;let t=r?`${r}/${e.getName()}`:e.getName();await c(e,t);continue}if(e instanceof t){o+=1;let t=r?`${r}/${e.getName()}`:e.getName();i.push(t)}}};return await c(e,``),T(`collectDatasetPaths`,Math.round(performance.now()-r),`${i.length} paths, ${a} dirs, ${o} files, ${s} listChildren calls`),i}function O(e){let t=[`severity`,`code`,`sub_code`,`location`,`message`],n={error:0,warn:1,warning:1,info:2};return{columns:t,rows:[...e].sort((e,t)=>{let r=(e.severity??`warn`).toLowerCase(),i=(t.severity??`warn`).toLowerCase(),a=n[r]??99,o=n[i]??99;if(a!==o)return a-o;let s=(e.code??``).localeCompare(t.code??``);return s===0?(e.location??``).localeCompare(t.location??``):s}).map(e=>[e.severity??`warn`,e.code??``,e.sub_code??``,e.location??``,e.message??``])}}async function k(e,t,n,r){return E(`python ${n}`,async()=>(await e.setGlobal(t,r),(await e.execCode(`
__args = ${t}.to_py()
await ${n}(__args['base_dir'], __args['paths'], __args.get('options'))
`)).result))}var A=new class{constructor(){this.pyEnvOperation=Promise.resolve()}resetPyEnv(){this.pyEnv?.close(),this.pyEnv=void 0,this.pyEnvReady=void 0}ensurePyEnvLogging(e){e.setStdoutCallback(e=>{let t=e.split(/\r?\n/).filter(e=>e.length>0);for(let e of t)console.log(e)}),e.setStderrCallback(e=>{let t=e.split(/\r?\n/).filter(e=>e.length>0);for(let e of t)console.error(e)})}async bootstrapPyEnv(e){let t=performance.now();console.log(`[ancpBIDS] bootstrapping PyEnv...`);let n=new d;return this.ensurePyEnvLogging(n),e?.({message:`Starting Python runtime...`}),await E(`pyenv.init`,()=>n.init()),await E(`pyenv ENV_NO_THREADS`,()=>n.execCode(v)),await E(`pyenv loadPackages(micropip)`,()=>n.loadPackages([`micropip`])),await E(`pyenv FCNTL_STUB`,()=>n.execCode(y)),e?.({message:`Installing ancpBIDS...`}),await E(`pyenv DOCKS_VFS`,()=>n.execCode(b)),await E(`pyenv INSTALL_ANCPBIDS`,()=>n.execCode(x)),await E(`pyenv VALIDATE_DATASET script`,()=>n.execCode(S)),await E(`pyenv LOAD_DATASET script`,()=>n.execCode(C)),T(`bootstrapPyEnv total`,Math.round(performance.now()-t)),n}getPyEnv(e){return this.pyEnv?(console.log(`[ancpBIDS] reusing warm PyEnv`),e?.({message:`Using Python runtime...`}),Promise.resolve(this.pyEnv)):(this.pyEnvReady||=this.bootstrapPyEnv(e).then(e=>(this.pyEnv=e,e)).catch(e=>{throw this.pyEnvReady=void 0,e}),this.pyEnvReady)}runWithPyEnv(e,t){let n=async()=>{let n=performance.now(),r=await E(`getPyEnv`,()=>this.getPyEnv(e));try{let e=await t(r);return T(`runWithPyEnv total`,Math.round(performance.now()-n)),e}catch(e){throw console.error(`[ancpBIDS] run failed, resetting PyEnv`),this.resetPyEnv(),e}},r=this.pyEnvOperation.then(n,n);return this.pyEnvOperation=r.then(()=>void 0,()=>void 0),r}async validateDataset(e,t,n){return this.runWithPyEnv(n,async r=>{console.log(`[ancpBIDS] validateDataset start root=${e.getName()}`),n?.({message:`Collecting dataset paths...`});let i=await D(e),a=e.getWorkspacePath();return n?.({message:`Loading and validating dataset...`}),k(r,`__ancpbids_validate_args`,`__ancpbids_validate_dataset`,{base_dir:a,paths:i,options:t??{}})})}async loadDatasetSummary(e,t,n){return this.runWithPyEnv(n,async r=>{console.log(`[ancpBIDS] loadDatasetSummary start root=${e.getName()}`),n?.({message:`Collecting dataset paths...`});let i=await D(e),a=e.getWorkspacePath();return n?.({message:`Loading dataset...`}),k(r,`__ancpbids_load_args`,`__ancpbids_load_dataset`,{base_dir:a,paths:i,options:t??{}})})}async runValidation(t,n){return await i.runAsync(`Validate BIDS dataset (ancpBIDS)`,async r=>{let i=await this.validateDataset(t,void 0,e=>{e.message!==void 0&&(r.message=e.message)}),a=t.getName()||`dataset`,{summary:o}=i,s=` (total ${o.total} · errors ${o.errors} · warnings ${o.warnings})`,c={title:n?.title??`${a}${s}`,source:n?.source??`ancpBIDS`,data:O(i.messages)};return e(u,c),i})}};c({name:`attribution.ancpbids`,label:`ancpBIDS`,component:()=>s`
    <p>
  ...
  `});var j=e=>e.getName()===`dataset_description.json`;function M(e){let n=e?.input?.data;return!(n instanceof t)||!j(n)?null:n.getParent()}a({command:{id:`ancpbids.validate`,name:`Validate BIDS dataset (ancpBIDS)`,description:`Loads and validates a BIDS dataset with ancpBIDS via Pyodide.`},handler:{canExecute:e=>!!M(e.activeEditor),execute:async e=>{let t=M(e.activeEditor);if(t)try{await A.runValidation(t)}catch(e){let t=e instanceof Error?e.message:String(e);o(`ancpBIDS validation failed: ${t}`)}}}}),r.registerContribution(l,{label:`ancpBIDS`,icon:`clipboard-list`,command:`ancpbids.validate`,ranking:10});