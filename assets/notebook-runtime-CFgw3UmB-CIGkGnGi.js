...
`,t=t.substring(p.length+1),a=!0),!a){let Z=this.rules.other.nextBulletRegex(f),te=this.rules.other.hrRegex(f),ne=this.rules.other.fencesBeginRegex(f),re=this.rules.other.headingBeginRegex(f),ve=this.rules.other.htmlBeginRegex(f);for(;t;){let F=t.split(`
`,1)[0],A;if(p=F,this.options.pedantic?(p=p.replace(this.rules.other.listReplaceNesting,"  "),A=p):A=p.replace(this.rules.other.tabCharGlobal,"    "),ne.test(p)||re.test(p)||ve.test(p)||Z.test(p)||te.test(p))break;if(A.search(this.rules.other.nonSpaceChar)>=f||!p.trim())h+=`
